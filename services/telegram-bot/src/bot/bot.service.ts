...
export class BotService implements OnModuleInit {
  ...
  private setupHandlers() {
        ...
        try {
          ...
          if (order && typeof order === "object") {
            ...
            const items = (order.items || []).map(
              (i: { name_ru: string; qty?: number; quantity?: number; price: number; code?: string }) => ({
                name_ru: i.name_ru,
    ...
    this.bot.on("callback_query", async (ctx) => {
      ...
      const original =
        cb.message && ("text" in cb.message || "caption" in cb.message)
          ? ((cb.message as { text?: string; caption?: string }).text ??
            (cb.message as { caption?: string }).caption ??
            "")
          : "";
